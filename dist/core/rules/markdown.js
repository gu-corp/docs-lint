    ...
    run(context) {
        ...
        for (const document of context.documents) {
            for (const match of document.body.matchAll(/(?<!!)\[[^\]]*\]\(([^)\s]+)(?:\s+['"][^'"]*['"])?\)/g)) {
                ...
                if (extension && !['.md', '.mdx', '.markdown', '.mdown', '.mkd'].includes(extension))
                    continue;
...
export const headingsRule = {
    ...
    run(context) {
        ...
        for (const document of context.documents) {
            ...
            if (h1.length === 0)
                diagnostics.push({ ruleId: '', severity: 'warning', file: document.path, message: 'Document has no H1 heading.' });
            if (h1.length > 1)
                diagnostics.push({ ruleId: '', severity: 'warning', file: document.path, location: { line: h1[1].line }, message: 'Document has more than one H1 heading.' });
...
export const codeFenceLanguageRule = {
        ...
        for (const document of context.documents) {
            ...
            for (let index = 0; index < lines.length; index += 1) {
                ...
                if (!fence) {
                    fence = marker;
                    if (!opening[2].trim())
                        diagnostics.push({ ruleId: '', severity: 'warning', file: document.path, location: { line: index + 1 }, message: 'Code fence has no language identifier.' });
                }

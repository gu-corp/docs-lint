...
export const standardStructureRule = {
    ...
    run(context) {
        if (!context.standardPack || !context.standardProfile)
            return [];
        const diagnostics = [];
        for (const folder of context.standardPack.manifest.structure || []) {
            if (folder.required && !context.pathExists(folder.path))
                diagnostics.push({ ruleId: '', severity: 'error', file: folder.path, message: 'Required documentation folder is missing.' });
        }

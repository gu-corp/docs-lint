 ...
 */
export async function checkBrokenLinks(docsDir, files, config) {
    const issues = [];
...
export async function checkTodoComments(docsDir, files, config) {
    const issues = [];
    // Parse config
    ...
    for (const file of files) {
        ...
        lines.forEach((line, lineNum) => {
            ...
            while ((match = todoPattern.exec(line)) !== null) {
                ...
                // Get tag config
                const tagConfig = tagConfigs[tag] || { severity: baseSeverity, message: tag };

...
export function registerInitCommand(program) {
    ...
    program.command('migrate')
        ...
        if (!isRecord(parsed))
            throw new Error('v2 configuration must be a JSON object.');
        ...
        for (const [oldId, newId] of Object.entries(mapping)) {
            ...
            if (value !== undefined)
                rules[newId] = migrateSeverity(value);
        }
        const migrated = {
            schemaVersion: 3,
...
function migrateSeverity(value) {
    if (typeof value === 'string')
        return value === 'warn' ? 'warning' : value;
    if (isRecord(value))
        return { ...value, severity: value.severity === 'warn' ? 'warning' : value.severity };
    return value;
}

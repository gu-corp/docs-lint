...
import fs$1, { lstatSync, readdir, readdirSync, readlinkSync, realpathSync } from "fs";
...
function normalizeConfig(value) {
	if (value.schemaVersion !== void 0 && value.schemaVersion !== 3) throw new Error(`Unsupported docs-lint configuration schema: ${value.schemaVersion}. Run docs-lint migrate.`);
	const config = {
		...DEFAULT_CONFIG,
		...value,
		schemaVersion: 3,
	...
	if (config.standard && (!config.standard.pack || typeof config.standard.pack !== "string")) throw new Error("standard.pack must be a non-empty string.");
	return config;
}
...
//#endregion
//#region dist/core/rules/content.js
const terminologyRule = {
	id: "content/terminology",
	description: "Detect configured non-preferred terminology.",
	defaultSeverity: "warning",
	run(context) {
		const terminology = [...context.standardPack?.manifest.terminology || [], ...context.config.terminology || []];
		const diagnostics = [];
		for (const document of context.documents) for (const term of terminology) for (const variant of term.variants) {
			let offset = 0;
			while ((offset = document.body.indexOf(variant, offset)) >= 0) {
				diagnostics.push({
					...
					fix: {
						description: `Replace with ${term.preferred}`,
						replacement: term.preferred
					}
				});
...
//#endregion
//#region dist/core/rules/markdown.js
const internalLinksRule = {
	...
	run(context) {
		const diagnostics = [];
		for (const document of context.documents) for (const match of document.body.matchAll(/(?<!!)\[[^\]]*\]\(([^)\s]+)(?:\s+['"][^'"]*['"])?\)/g)) {
			...
			const extension = path$1.posix.extname(decoded).toLowerCase();
			if (extension && ![
				".md",
				".mdx",
				".markdown",
			...
			if (!(extension ? [target] : [
				...
				`${target}/index.md`,
				`${target}/index.mdx`
			]).some((candidate) => context.documentPaths.has(candidate))) diagnostics.push(issue(document, match.index, `Linked document does not exist: ${href}`));
		}
		return diagnostics;
	}
};
const headingsRule = {
	...
	run(context) {
		...
		for (const document of context.documents) {
			...
			if (h1.length === 0) diagnostics.push({
				ruleId: "",
				severity: "warning",
				file: document.path,
				message: "Document has no H1 heading."
			});
			if (h1.length > 1) diagnostics.push({
				ruleId: "",
				severity: "warning",
				file: document.path,
				location: { line: h1[1].line },
				message: "Document has more than one H1 heading."
			});
			for (let index = 1; index < headings.length; index += 1) if (headings[index].level > headings[index - 1].level + 1) diagnostics.push({
				ruleId: "",
				severity: "warning",
				file: document.path,
				location: { line: headings[index].line },
				message: `Heading level skips from H${headings[index - 1].level} to H${headings[index].level}.`
			});
		}
		return diagnostics;
	}
};
const codeFenceLanguageRule = {
	...
	run(context) {
		...
		for (const document of context.documents) {
			...
			for (let index = 0; index < lines.length; index += 1) {
				...
				if (!fence) {
					fence = marker;
					if (!opening[2].trim()) diagnostics.push({
						ruleId: "",
						severity: "warning",
						file: document.path,
						location: { line: index + 1 },
						message: "Code fence has no language identifier."
					});
				} else if (fence === marker) fence = void 0;
			}
		}
		return diagnostics;
	}
...
function markdownHeadings(markdown) {
	...
	for (const [index, line] of markdown.split(/\r?\n/).entries()) {
		...
		if (fenceMatch) {
			fence = fence ? void 0 : fenceMatch[1][0];
			continue;
		}
		if (fence) continue;
...
function normalizeTarget(current, target) {
	const joined = target.startsWith("/") ? target.slice(1) : path$1.posix.join(path$1.posix.dirname(current), target);
	return path$1.posix.normalize(joined).replace(/^\.\//, "");
}
function issue(document, index, message) {
	const line = index === void 0 ? void 0 : document.body.slice(0, index).split(/\r?\n/).length;
	return {
		ruleId: "",
		severity: "error",
		file: document.path,
		location: line ? { line } : void 0,
		message
	};
}
//#endregion
//#region dist/core/rules/standard.js
const standardStructureRule = {
	...
	run(context) {
		...
		const diagnostics = [];
		for (const folder of context.standardPack.manifest.structure || []) if (folder.required && !context.pathExists(folder.path)) diagnostics.push({
			ruleId: "",
			severity: "error",
			file: folder.path,
			message: "Required documentation folder is missing."
		});
		for (const typeId of context.standardProfile.requiredDocuments || []) {
			...
			if (type?.suggestedPath && !context.documentPaths.has(type.suggestedPath)) diagnostics.push({
				ruleId: "",
				severity: "error",
				file: type.suggestedPath,
				message: `Required document is missing for type ${typeId}.`,
				data: { documentType: typeId }
			});
		}
		return diagnostics;
	}
...
const requiredSectionsRule = {
	...
	run(context) {
		...
		const diagnostics = [];
		for (const document of context.documents) {
			const typeId = document.frontMatter.documentType || Object.entries(context.standardPack.manifest.documentTypes).find(([, type]) => type.suggestedPath === document.path)?.[0];
			if (!typeId) continue;
			const type = context.standardPack.manifest.documentTypes[typeId];
			if (!type) {
				diagnostics.push({
					...
					severity: "warning",
					file: document.path,
					message: `Unknown documentType in front matter: ${typeId}`
				});
				continue;
			}
			const headings = markdownHeadings(document.body);
			for (const section of type.sections || []) {
				...
				if (section.required && !headings.some((heading) => titles.includes(heading.title) && (!section.level || heading.level === section.level))) diagnostics.push({
					ruleId: "",
					severity: "warning",
					file: document.path,
					message: `Required section is missing: ${titles[0]}`,
					data: {
						documentType: typeId,
...
minimatch.sep = defaultPlatform$2 === "win32" ? path$2.win32.sep : path$2.posix.sep;
...
//#endregion
//#region dist/core/rules/traceability.js
const DEFAULT_REQUIREMENT_PREFIXES = [
	"BR",
	"FR",
	"SR",
	"NFR"
...
const requirementsTestsRule = {
	...
	run(context) {
		...
		if (setup.requirementFiles.length === 0) return [];
		const diagnostics = [];
		const config = setup.config;
		if (config.requireRequirementIds !== false) {
			for (const document of setup.requirementFiles) if (!hasMatch(maskCode(document.body), setup.requirementPattern)) diagnostics.push({
				ruleId: "",
				severity: "warning",
				file: document.path,
				location: {
				...
				message: "Requirement document has no requirement IDs."
			});
		}
		if (setup.requirements.size === 0) return diagnostics;
		if (setup.testFiles.length === 0) {
			if (config.requireTestFile !== false) diagnostics.push({
				ruleId: "",
				severity: "warning",
				message: "Requirements exist but no test document was found.",
				data: { testFiles: config.testFiles || DEFAULT_TEST_FILES }
			});
			return diagnostics;
		}
		if (config.requireTestCaseIds !== false) {
			for (const document of setup.testFiles) if (!hasMatch(maskCode(document.body), setup.testPattern)) diagnostics.push({
				ruleId: "",
				severity: "warning",
				file: document.path,
				location: {
				...
				message: "Test document has no test case IDs."
			});
		}
		const testLines = setup.testFiles.flatMap((document) => maskCode(document.body).split(/\r?\n/));
		if (!testLines.some((line) => hasMatch(line, setup.testPattern))) diagnostics.push({
			ruleId: "",
			severity: "warning",
			message: "Requirements exist but no test case IDs were found."
		});
		let covered = 0;
		let excluded = 0;
		for (const [id, definition] of setup.requirements) {
			const status = classify(testLines.filter((line) => line.includes(id)), setup);
			if (status === "covered") {
				covered += 1;
				continue;
			}
		...
		if (requiredCoverage !== void 0 && actualCoverage < requiredCoverage) diagnostics.push({
			ruleId: "",
			severity: "warning",
			...
			data: {
				actualCoverage,
				requiredCoverage,
				requirements: setup.requirements.size,
				covered,
				excluded
			}
		});
	...
	run(context) {
		...
		for (const document of setup.requirementFiles) {
			...
			for (const match of masked.matchAll(loose)) {
				if (isStrictMatch(match[0], setup.requirementPattern)) continue;
				diagnostics.push({
					ruleId: "",
					severity: "warning",
					message: `Requirement ID does not follow the configured pattern: ${match[0]}`,
					file: document.path,
					location: positionAt(document, match.index),
		...
		if (setup.config.requireUniqueRequirementIds !== false) {
			...
			for (const document of setup.requirementFiles) for (const occurrence of definitionOccurrences(document, setup.requirementPattern)) {
...
const requirementReferencesRule = {
	...
	run(context) {
		...
		for (const document of [...setup.testFiles, ...setup.otherFiles]) {
			...
			for (const match of masked.matchAll(freshRegExp(setup.requirementPattern))) {
				const id = match[0];
				if (setup.requirements.has(id)) continue;
...
function prepare(context) {
	...
	const prefixes = (config.requirementPrefixes && config.requirementPrefixes.length > 0 ? config.requirementPrefixes : DEFAULT_REQUIREMENT_PREFIXES).map((prefix) => prefix.toUpperCase());
	...
	return {
		...
		requirementFiles,
		testFiles,
		otherFiles: context.documents.filter((document) => !requirementPaths.has(document.path) && !testPaths.has(document.path)),
...
function classify(references, setup) {
	if (references.length === 0) return "uncovered";
	let deferred = false;
	let excluded = false;
	for (const line of references) {
		const testCases = line.match(freshRegExp(setup.testPattern)) || [];
		if (testCases.length === 0) return "covered";
		let lineDeferred = false;
		let lineExcluded = false;
		for (const testCase of testCases) if (hasMatch(testCase, setup.excludedPattern)) lineExcluded = true;
		else if (hasMatch(testCase, setup.deferredPattern)) lineDeferred = true;
		else return "covered";
...
function occurrenceAt(document, index) {
	...
	return {
		file: document.path,
		location: positionAt(document, index),
		line,
		index
	};
}
...
function matchesAny(file, patterns) {
	return patterns.some((pattern) => new Minimatch(pattern, {
		nocase: true,
		dot: true
	}).match(file));
}
//#endregion
//#region dist/core/engine.js
const BUILTIN_RULES = [
	internalLinksRule,
	headingsRule,
...
function loadConfig(options = {}) {
	...
	const configPath = options.configPath ? path$1.resolve(cwd, options.configPath) : options.search === false ? void 0 : findUp(cwd, CONFIG_FILENAME$1);
	let value = {};
	let configDirectory = cwd;
	if (configPath) {
		configDirectory = path$1.dirname(configPath);
		const parsed = readJson(configPath);
		if (!isRecord$1(parsed)) throw new Error(`docs-lint configuration must be a JSON object: ${configPath}`);
		if (parsed.schemaVersion !== 3) throw new Error(`docs-lint v3 requires schemaVersion: 3 in ${configPath}. Run docs-lint migrate.`);
		if ("docsDir" in parsed) throw new Error("docsDir was replaced by root in docs-lint v3. Run docs-lint migrate.");
		validateConfigShape(parsed);
		value = parsed;
	}
	if (options.root) value.root = options.root;
	const normalized = normalizeConfig(value);
	const rootPath = path$1.resolve(configDirectory, normalized.root || DEFAULT_CONFIG.root);
...
function readLunascapeStandard(rootPath) {
	...
	if (typeof source !== "string" || !source.trim()) throw new Error(`${configPath}: documentStandards.pack must be a non-empty string.`);
	if (profile !== void 0 && (typeof profile !== "string" || !profile.trim())) throw new Error(`${configPath}: documentStandards.profile must be a non-empty string.`);
	return {
		pack: source.startsWith("builtin:") || !path$1.isAbsolute(source) && /^[a-z][a-z0-9+.-]*:/i.test(source) ? source : path$1.resolve(rootPath, source),
...
function validateConfigShape(value) {
	...
	if (unknown.length) throw new Error(`Unknown docs-lint v3 configuration properties: ${unknown.join(", ")}`);
	if (typeof value.root !== "string" || !value.root.trim()) throw new Error("root must be a non-empty string.");
	for (const key of ["include", "exclude"]) if (value[key] !== void 0 && (!Array.isArray(value[key]) || value[key].some((item) => typeof item !== "string"))) throw new Error(`${key} must be an array of strings.`);
	if (value.standard !== void 0) {
		if (!isRecord$1(value.standard) || typeof value.standard.pack !== "string" || !value.standard.pack.trim()) throw new Error("standard.pack must be a non-empty string.");
		if (value.standard.profile !== void 0 && (typeof value.standard.profile !== "string" || !value.standard.profile.trim())) throw new Error("standard.profile must be a non-empty string.");
	}
	if (value.rules !== void 0 && !isRecord$1(value.rules)) throw new Error("rules must be an object.");
	if (value.terminology !== void 0 && !Array.isArray(value.terminology)) throw new Error("terminology must be an array.");
	if (value.traceability !== void 0) {
		if (!isRecord$1(value.traceability)) throw new Error("traceability must be an object.");
		const coverage = value.traceability.requiredCoverage;
		if (coverage !== void 0 && (typeof coverage !== "number" || coverage < 0 || coverage > 1)) throw new Error("traceability.requiredCoverage must be between 0 and 1.");
	}
}
...
function findUp(start, fileName) {
	let directory = start;
	while (true) {
		const candidate = path$1.join(directory, fileName);
...
//#endregion
//#region dist/standards/manifest.js
...
function validateStandardPackManifest(value) {
	const issues = [];
	if (!isRecord(value)) return ["Manifest must be a JSON object."];
	const pack = value;
	if (pack.schemaVersion !== 1) issues.push("schemaVersion must be 1.");
	if (typeof pack.id !== "string" || !ID_PATTERN.test(pack.id)) issues.push("id must use lowercase URL-safe segments.");
	if (typeof pack.version !== "string" || !VERSION_PATTERN.test(pack.version)) issues.push("version must use semantic versioning.");
	if (!isLocalizedText(pack.title)) issues.push("title must be a string or locale map.");
	if (pack.description !== void 0 && !isLocalizedText(pack.description)) issues.push("description must be a string or locale map.");
	if (!isRecord(pack.profiles) || Object.keys(pack.profiles).length === 0) issues.push("profiles must be a non-empty object.");
	...
	if (pack.defaultProfile !== void 0 && (typeof pack.defaultProfile !== "string" || !profiles[pack.defaultProfile])) issues.push(`defaultProfile does not exist: ${String(pack.defaultProfile)}`);
	validateRules("Pack", pack.rules, issues);
	for (const [profileId, candidate] of Object.entries(profiles)) {
		if (!ID_PATTERN.test(profileId)) issues.push(`Invalid profile id: ${profileId}`);
		if (!isRecord(candidate)) {
			issues.push(`Profile ${profileId} must be an object.`);
			continue;
		}
		const profile = candidate;
		if (!isLocalizedText(profile.title)) issues.push(`Profile ${profileId} requires a title.`);
		...
		for (const parent of parents) if (!profiles[parent]) issues.push(`Profile ${profileId} extends missing profile: ${parent}`);
		for (const type of [...enabled, ...required]) if (!documentTypes[type]) issues.push(`Profile ${profileId} references missing document type: ${type}`);
		if (profile.variables !== void 0 && (!isRecord(profile.variables) || Object.values(profile.variables).some((item) => typeof item !== "string"))) issues.push(`Profile ${profileId}.variables must contain string values.`);
	...
	for (const [id, candidate] of Object.entries(documentTypes)) {
		if (!ID_PATTERN.test(id)) issues.push(`Invalid document type id: ${id}`);
		if (!isRecord(candidate)) {
			issues.push(`Document type ${id} must be an object.`);
			continue;
		}
		const documentType = candidate;
		if (!isLocalizedText(documentType.title)) issues.push(`Document type ${id} requires a title.`);
		if (typeof documentType.template !== "string" || !isSafeRelativePath(documentType.template)) issues.push(`Document type ${id}.template must be a safe relative path.`);
		if (documentType.suggestedPath !== void 0 && (typeof documentType.suggestedPath !== "string" || !isSafeRelativePath(documentType.suggestedPath))) issues.push(`Document type ${id}.suggestedPath must be a safe relative path.`);
		if (documentType.variables !== void 0) validateVariables(id, documentType.variables, issues);
		if (documentType.sections !== void 0) validateSections(id, documentType.sections, issues);
	}
	if (pack.structure !== void 0) {
		if (!Array.isArray(pack.structure)) issues.push("structure must be an array.");
		else for (const [index, folder] of pack.structure.entries()) if (!isRecord(folder) || typeof folder.path !== "string" || !isSafeRelativePath(folder.path)) issues.push(`structure[${index}].path must be a safe relative path.`);
	}
	if (pack.terminology !== void 0) validateTerminology(pack.terminology, issues);
	return issues;
}
function resolveStandardProfile(pack, profileId) {
	const id = profileId || pack.defaultProfile || Object.keys(pack.profiles)[0];
	if (!id || !pack.profiles[id]) throw new Error(`Standard profile does not exist: ${id || "(none)"}`);
	const visiting = /* @__PURE__ */ new Set();
	const inheritance = [];
	function visit(currentId) {
		if (visiting.has(currentId)) throw new Error(`Standard profile inheritance cycle: ${[...visiting, currentId].join(" -> ")}`);
		const current = pack.profiles[currentId];
		if (!current) throw new Error(`Standard profile does not exist: ${currentId}`);
		visiting.add(currentId);
		let merged = { title: current.title };
		for (const parent of current.extends || []) merged = mergeProfile(merged, visit(parent));
	...
	return {
		...visit(id),
		id,
		inheritance: [...new Set(inheritance)]
	};
}
...
function detectCycles(profiles, issues) {
	...
	function visit(id, path) {
		...
		if (done.has(id) || !profiles[id]) return;
		active.add(id);
		for (const parent of Array.isArray(profiles[id].extends) ? profiles[id].extends : []) visit(parent, [...path, id]);
		active.delete(id);
		done.add(id);
	}
	for (const id of Object.keys(profiles)) visit(id, []);
}
...
function validateVariables(id, value, issues) {
	...
	for (const [name, variable] of Object.entries(value)) if (!/^[A-Za-z][A-Za-z0-9_.-]*$/.test(name) || !isRecord(variable)) issues.push(`Document type ${id} has an invalid variable: ${name}`);
	else if (variable.default !== void 0 && typeof variable.default !== "string") issues.push(`Document type ${id}.variables.${name}.default must be a string.`);
}
function validateSections(id, value, issues) {
	...
	for (const [index, section] of value.entries()) {
		...
		if (sectionIds.has(section.id)) issues.push(`Document type ${id} has duplicate section id: ${section.id}`);
		sectionIds.add(section.id);
		if (!isLocalizedText(section.title)) issues.push(`Document type ${id}.sections.${section.id} requires a title.`);
		if (section.level !== void 0 && (!Number.isInteger(section.level) || section.level < 1 || section.level > 6)) issues.push(`Document type ${id}.sections.${section.id}.level must be 1-6.`);
	}
}
function validateTerminology(value, issues) {
	...
	for (const [index, term] of value.entries()) {
		if (!isRecord(term) || typeof term.preferred !== "string" || !term.preferred.trim()) issues.push(`terminology[${index}].preferred is required.`);
		if (!isRecord(term) || !Array.isArray(term.variants) || term.variants.length === 0 || term.variants.some((item) => typeof item !== "string" || !item)) issues.push(`terminology[${index}].variants must be a non-empty string array.`);
	}
}
...
function isLocalizedText(value) {
	return typeof value === "string" ? Boolean(value.trim()) : isRecord(value) && Object.values(value).length > 0 && Object.values(value).every((item) => typeof item === "string" && item.trim());
}
function unique(values) {
	return [...new Set(values)];
}
//#endregion
//#region ../../node_modules/lru-cache/dist/esm/node/index.min.js
...
const normalizeCache = new I({ max: 4096 });
const normalize = (s) => {
...
const normalizeNocaseCache = new I({ max: 4096 });
...
glob.glob = glob;
//#endregion
//#region dist/node/workspace.js
async function loadDocuments(config) {
	if (!fs$1.existsSync(config.rootPath) || !fs$1.statSync(config.rootPath).isDirectory()) throw new Error(`Documentation root was not found: ${config.rootPath}`);
	const matches = /* @__PURE__ */ new Set();
	for (const pattern of config.include) for (const file of await glob(pattern, {
		cwd: config.rootPath,
		ignore: config.exclude,
		nodir: true,
		follow: false
	})) matches.add(toPosix$1(file));
	const documents = [];
...
function securePath(rootPath, relativePath, requireExisting = true) {
	const root = path$1.resolve(rootPath);
	const target = path$1.resolve(root, relativePath);
	const relative = path$1.relative(root, target);
	if (relative === ".." || relative.startsWith(`..${path$1.sep}`) || path$1.isAbsolute(relative)) throw new Error(`Path escapes documentation root: ${relativePath}`);
	if (requireExisting) {
		if (!fs$1.existsSync(target)) throw new Error(`Path does not exist: ${relativePath}`);
		assertRealPathWithinRoot(root, target, relativePath);
	} else assertRealPathWithinRoot(root, nearestExistingPath(target), relativePath);
	return target;
}
...
function assertRealPathWithinRoot(rootPath, candidatePath, label) {
	const root = fs$1.realpathSync(rootPath);
	const candidate = fs$1.realpathSync(candidatePath);
	const relative = path$1.relative(root, candidate);
	if (relative === ".." || relative.startsWith(`..${path$1.sep}`) || path$1.isAbsolute(relative)) throw new Error(`Path symlink escapes documentation root: ${label}`);
}
//#endregion
//#region dist/node/standard-pack.js
const VARIABLE_PATTERN = /\{\{\s*([A-Za-z][A-Za-z0-9_.-]*)\s*\}\}/g;
function loadConfiguredStandard(config) {
	if (!config.standard) return {};
	const pack = loadStandardPack(resolvePackSource(config.standard.pack, config.configDirectory));
	return {
		standardPack: pack,
		standardProfile: resolveStandardProfile(pack.manifest, config.standard.profile)
	};
}
function loadStandardPack(source) {
	...
	for (const [id, type] of Object.entries(manifest.documentTypes || {})) try {
...
function renderTemplate(pack, profile, documentTypeId, supplied = {}) {
	...
	if (profile.documentTypes?.length && !profile.documentTypes.includes(documentTypeId)) throw new Error(`Document type ${documentTypeId} is not enabled by profile ${profile.id}.`);
	const variables = {
	...
	for (const [name, definition] of Object.entries(documentType.variables || {})) if (definition.default !== void 0) variables[name] = definition.default;
	Object.assign(variables, supplied);
	const missing = Object.entries(documentType.variables || {}).filter(([name, definition]) => definition.required && !variables[name]?.trim()).map(([name]) => name);
	if (missing.length) throw new Error(`Required template variables are missing: ${missing.join(", ")}`);
	const unresolved = /* @__PURE__ */ new Set();
	const rendered = fs$1.readFileSync(securePackFile(pack.rootPath, documentType.template), "utf8").replace(VARIABLE_PATTERN, (_match, name) => {
...
function bundledPacksRoot() {
	const moduleDirectory = path$1.dirname(fileURLToPath(import.meta.url));
	const parentName = path$1.basename(path$1.dirname(moduleDirectory));
	const packageRoot = path$1.basename(moduleDirectory) === "node" && (parentName === "src" || parentName === "dist") ? path$1.resolve(moduleDirectory, "..", "..") : path$1.resolve(moduleDirectory, "..");
	return path$1.join(packageRoot, "packs");
}
function resolvePackSource(source, configDirectory) {
	if (source.startsWith("builtin:")) {
		const packName = source.slice(8);
		if (!/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(packName) || packName === "." || packName === "..") throw new Error(`Invalid built-in standard pack name: ${packName || "(empty)"}`);
		return securePath(bundledPacksRoot(), packName, false);
	}
	if (!path$1.isAbsolute(source) && /^[a-z][a-z0-9+.-]*:/i.test(source)) throw new Error(`Unsupported standard pack source: ${source}. v3 accepts builtin: or local paths.`);
	return path$1.resolve(configDirectory, source);
}
function securePackFile(rootPath, relativePath) {
	const candidate = securePath(rootPath, relativePath, false);
	if (!fs$1.existsSync(candidate) || !fs$1.statSync(candidate).isFile()) throw new Error(`Pack file does not exist: ${relativePath}`);
	const root = fs$1.realpathSync(rootPath);
	const real = fs$1.realpathSync(candidate);
	const relative = path$1.relative(root, real);
	if (relative === ".." || relative.startsWith(`..${path$1.sep}`) || path$1.isAbsolute(relative)) throw new Error(`Pack file symlink escapes the pack root: ${relativePath}`);
	return real;
}
//#endregion
//#region dist/node/run.js
async function lintWorkspace(config, options = {}, standard = loadConfiguredStandard(config)) {
	const documents = await loadDocuments(config);
	return new DocsLintEngine().lint({
		config,
		documents,
		...standard,
		only: options.only,
		...
		pathExists(relativePath) {
			try {
				return fs$1.existsSync(securePath(config.rootPath, relativePath, false));
			} catch {
				return false;
			}
		}
...
function createNodeDocsLintSession(options) {
	...
	return {
		...
		renderTemplate(templateId, variables = {}) {
			if (!standard.standardPack || !standard.standardProfile) throw new Error("No Standard Pack is configured for this documentation root.");
...
function resolveSessionConfigPath(options, workspaceRoot, docsRoot) {
	if (options.configPath !== void 0) {
		...
		if (!fs.existsSync(configPath) || !fs.statSync(configPath).isFile()) throw new Error(`docs-lint configuration was not found: ${configPath}`);
		return configPath;
	}
	...
	while (true) {
		...
		if (directory === workspaceRoot) return void 0;
...
function assertLocalStandardWithinWorkspace(workspaceRoot, config) {
	...
	assertPathWithin(workspaceRoot, path.isAbsolute(source) ? path.resolve(source) : path.resolve(config.configDirectory, source), "standard.pack");
}
function loadSessionConfig(docsRoot, configPath) {
	...
	const config = loadConfig({
		cwd: configDirectory,
		configPath,
		root: toPosix(path.relative(configDirectory, docsRoot)) || ".",
		search: false
	});
	if (path.resolve(config.rootPath) !== docsRoot) throw new Error(`Failed to bind docs-lint configuration to documentation root: ${docsRoot}`);
	return config;
}
function describeRules(config, pack, profile) {
	return BUILTIN_RULES.map((rule) => {
		const candidates = [
			{
				setting: config.rules[rule.id],
				source: "config"
			},
			{
				setting: profile?.rules?.[rule.id],
				source: "profile"
			},
			{
				setting: pack?.manifest.rules?.[rule.id],
				source: "pack"
			}
		];
...
function describeTemplates(pack, profile) {
	if (!pack || !profile) return [];
	return (profile.documentTypes || Object.keys(pack.manifest.documentTypes)).map((id) => {
		...
		return {
			...
			variables: Object.fromEntries(Object.entries(template.variables || {}).map(([name, definition]) => [name, { ...definition }]))
		};
	});
}
...
function assertDirectory(candidate, label) {
	if (!fs.existsSync(candidate) || !fs.statSync(candidate).isDirectory()) throw new Error(`${label} was not found: ${candidate}`);
}
function assertPathWithin(root, candidate, label) {
	const relative = path.relative(root, candidate);
	if (relative === ".." || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)) throw new Error(`${label} must be inside workspaceRoot: ${candidate}`);
	if (!fs.existsSync(candidate)) return;
	const realRoot = fs.realpathSync(root);
	const realCandidate = fs.realpathSync(candidate);
	const realRelative = path.relative(realRoot, realCandidate);
	if (realRelative === ".." || realRelative.startsWith(`..${path.sep}`) || path.isAbsolute(realRelative)) throw new Error(`${label} symlink must resolve inside workspaceRoot: ${candidate}`);
}
function toPosix(value) {
	return value.split(path.sep).join("/");
}
//#endregion
export { createNodeDocsLintSession };

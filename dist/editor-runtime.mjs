...
const requirementsTestsRule = {
	...
	run(context) {
		...
		const diagnostics = [];
		for (const id of requirements) if (!testText.includes(id)) diagnostics.push({
			ruleId: "",
			severity: "warning",
...
const normalizeCache = new LRUCache({ max: 4096 });
const normalize = (s) => {
...
const normalizeNocaseCache = new LRUCache({ max: 4096 });

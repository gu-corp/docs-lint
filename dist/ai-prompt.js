 ...
 */
export function readDocsConfig(docsDir) {
 ...
 */
export function readStandardsFile(docsDir, useDefault = true) {
 ...
 */
export function generateAIPrompt(docsDir, files, lintResult, config) {
    const sections = [];
 ...
 */
function generateFolderTree(docsDir, files) {
    const tree = new Map();
 ...
 */
function formatIssues(ruleResults) {
    const lines = [];
 ...
 */
export function generateJSONSummary(docsDir, files, lintResult) {
    return {
        meta: {
        ...
        issues: lintResult.ruleResults.flatMap((r) => r.issues.map((i) => ({
            rule: r.rule,
            severity: r.severity,
            file: i.file,
            line: i.line,
        ...
        files: files.map((f) => ({
            path: f,
            issues: lintResult.ruleResults.flatMap((r) => r.issues.filter((i) => i.file === f).map((i) => ({
                rule: r.rule,
                message: i.message,
            }))),
        })),

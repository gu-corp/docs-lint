...
export function registerCreateCommand(program: Command): void {
  program.command('create <document-type>')
    ...
    .action((documentTypeId, options) => {
      ...
      if (!pack || !profile) throw new Error('No Standard Pack is configured.');
      ...
      if (!output) throw new Error('Output is required because this document type has no suggestedPath.');
...
function assignments(values: string[]): Record<string, string> {
  const result: Record<string, string> = {};
  for (const value of values) {
    const index = value.indexOf('=');
    if (index <= 0) throw new Error(`Template variable must use key=value: ${value}`);

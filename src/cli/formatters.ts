...
export function printResults(
  ...
  const hasMarkdownLintIssues = result.ruleResults.some(
    r => r.rule === 'markdownLint' && r.issues.length > 0
  );
 ...
 */
export function printCodeCheckResults(
  result: CheckResult,
  verbose?: boolean
): void {
  console.log(chalk.bold('\n🔍 Code Check Results\n'));
 ...
 */
export function printCoverageReport(report: CoverageReport, verbose?: boolean): void {

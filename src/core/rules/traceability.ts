...
import type { Diagnostic, DiagnosticLocation, DocumentFile, RuleContext, RuleDefinition, TraceabilityConfig } from '../types.js';
...
interface TraceabilitySetup {
  config: TraceabilityConfig;
  prefixes: string[];
  requirementPattern: RegExp;
  testPattern: RegExp;
  deferredPattern: RegExp;
  excludedPattern: RegExp;
  requirementFiles: DocumentFile[];
  testFiles: DocumentFile[];
  otherFiles: DocumentFile[];
  /** First definition of every requirement ID found in requirement documents. */
  requirements: Map<string, Occurrence>;
}
...
export const requirementIdsRule: RuleDefinition = {
...
export const requirementReferencesRule: RuleDefinition = {
...
function prepare(context: RuleContext): TraceabilitySetup {
...
function defaultRequirementPattern(prefixes: string[]): string {
...
function classify(references: string[], setup: TraceabilitySetup): 'covered' | 'deferred' | 'excluded' | 'uncovered' {
  ...
  for (const line of references) {
    ...
    for (const testCase of testCases) {
      if (hasMatch(testCase, setup.excludedPattern)) lineExcluded = true;
...
function freshRegExp(regex: RegExp): RegExp {
...
function locate(documents: DocumentFile[], regex: RegExp): Map<string, Occurrence> {
...
function definitionOccurrences(document: DocumentFile, regex: RegExp): Occurrence[] {
  const masked = maskCode(document.body);
...
function occurrenceAt(document: DocumentFile, index: number): Occurrence {
...
function indexOfLineEnd(text: string, index: number): number {
  const end = text.indexOf('\n', index);
  return end < 0 ? text.length : end;
}

function positionAt(document: DocumentFile, bodyIndex: number): DiagnosticLocation {

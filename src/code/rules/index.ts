...
import type {
  CheckIssue,
  RuleSeverity,
  TestFileExistsConfig,
  TestTypeCoverageConfig,
  CriticalPath,
  TestPatterns,
} from '../types.js';
 ...
 */
export async function checkTestFileExists(
  srcDir: string,
  sourceFiles: string[],
  testFiles: string[],
  config: TestFileExistsConfig | RuleSeverity
): Promise<CheckIssue[]> {
 ...
 */
export async function checkTestTypeCoverage(
  srcDir: string,
  sourceFiles: string[],
  testPatterns: TestPatterns,
  testDirs: string[],
  config: TestTypeCoverageConfig | RuleSeverity
): Promise<CheckIssue[]> {
  const issues: CheckIssue[] = [];
 ...
 */
export async function checkCriticalE2E(
  srcDir: string,
  sourceFiles: string[],
  e2ePattern: string,
  testDirs: string[],
  ...
  for (const criticalPath of criticalPaths) {
    ...
    const matchingFiles = sourceFiles.filter((file) =>
      criticalPath.patterns.some((pattern) => {
        const regex = new RegExp(pattern.replace(/\*\*/g, '.*').replace(/\*/g, '[^/]*'));
        return regex.test(file);
      })
    );
    ...
    const hasE2E = e2eTests.some((test) => {
      ...
      return (
        test.toLowerCase().includes(criticalPath.name.toLowerCase()) ||
        criticalPath.patterns.some((p) =>
          testContent.includes(p.replace(/\*\*/g, '').replace(/\*/g, ''))
        )
      );
    });

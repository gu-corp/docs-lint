...
---`);
  ...
  // MECE check section
  sections.push(`## MECE（相互排他・完全網羅）チェック

ドキュメント全体がMECE（Mutually Exclusive, Collectively Exhaustive）の原則に従っているかを評価してください。
...
チェック項目:
- [ ] **重複**: 同じ機能・仕様が複数ドキュメントに記載されていないか
- [ ] **矛盾**: ドキュメント間で異なる仕様が記述されていないか
- [ ] **責務の重複**: 複数コンポーネントに同じ責務が割り当てられていないか
- [ ] **用語の不統一**: 同じ概念に異なる用語が使われていないか

検出すべきパターン:
| パターン | 例 | 問題点 |
|---------|-----|-------|
| 仕様の重複 | REQUIREMENTS.mdとAPI.mdで同じ制限値を別々に定義 | 更新漏れのリスク |
| 責務の重複 | 「入力検証はControllerで行う」と「入力検証はServiceで行う」 | どちらが正か不明 |
| 定義の矛盾 | 「タイムアウトは30秒」と「タイムアウトは60秒」 | 実装時に混乱 |
| 用語の揺れ | 「ユーザー」「利用者」「顧客」が混在 | 同じものか別か不明 |
...
チェック項目:
- [ ] **機能の網羅**: 全ユースケースがカバーされているか
- [ ] **状態の網羅**: 全ての状態遷移が定義されているか
- [ ] **エラーの網羅**: 起こりうる全エラーが定義されているか
- [ ] **境界の網羅**: 入力値の境界条件が全て定義されているか
- [ ] **役割の網羅**: 全ユーザー種別のフローが定義されているか

検出すべきギャップ:
| カテゴリ | チェック観点 | 欠落例 |
|---------|-------------|-------|
| CRUD | 作成・読取・更新・削除すべてあるか | 「削除」の仕様がない |
| ユーザー種別 | 管理者・一般・ゲストすべてあるか | 「ゲストユーザー」の動作が未定義 |
| 状態遷移 | 全状態間の遷移が定義されているか | 「キャンセル済→再開」の遷移が未定義 |
| API | 必要なエンドポイントが揃っているか | 一覧取得APIはあるが詳細取得APIがない |
| エラー | 想定されるエラーすべてにコードがあるか | DBエラー時の動作が未定義 |

### ドキュメント構成のMECE

以下のフォルダ構成で、各ドキュメントの責務が明確に分離されているか確認：
...
│   └── 04-testing/       → テスト仕様
├── 03-guide/         → どう使うか（How to use）
└── 04-development/   → どう作るか（How to build）
\`\`\`

チェック項目:
- [ ] 各フォルダに適切なドキュメントが配置されているか
- [ ] ドキュメント間の参照関係が明確か
- [ ] 情報の「単一の情報源（Single Source of Truth）」が確立されているか
...
| チェック項目 | 結果 | 詳細 |
|-------------|------|------|
| 仕様の重複 | ✅なし/⚠️軽微/❌あり | [検出された重複] |
| 定義の矛盾 | ✅なし/⚠️軽微/❌あり | [検出された矛盾] |
| 機能の網羅 | ✅完全/⚠️一部欠落/❌不足 | [欠落している機能] |
| エラー定義の網羅 | ✅完全/⚠️一部欠落/❌不足 | [未定義のエラーケース] |
| 用語の統一 | ✅統一/⚠️一部揺れ/❌混在 | [揺れている用語] |
 ...
 */
export function generateCodeReviewPrompt(options: CodeReviewPromptOptions): string {
